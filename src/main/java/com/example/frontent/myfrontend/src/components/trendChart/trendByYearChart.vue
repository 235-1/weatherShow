...
const initChart = () => {
  ...
  const option = {
    ...
    legend: { data: ['最高温', '平均温', '最低温'], textStyle: { color: '#fff' } },
    // 关键点：将 bottom 设置为 80px，强制让绘图区和下面的图对齐
    grid: { top: '80px', left: '5%', right: '5%', bottom: '80px', containLabel: true },
    xAxis: { type: 'category', data: [], axisLabel: { color: '#fff' } },
    yAxis: { type: 'value', name: '温度 (°C)', nameTextStyle: { color: '#fff' }, axisLabel: { color: '#fff' } },
    series: [
      { name: '最高温', type: 'line', itemStyle: { color: '#ff4d4f' }, data: [] },
      { name: '平均温', type: 'line', smooth: true, areaStyle: { opacity: 0.1 }, itemStyle: { color: '#1890ff' }, data: [] },
      { name: '最低温', type: 'line', itemStyle: { color: '#52c41a' }, data: [] }
    ]
  }
