...
import { getDiurnalDeviation,getSeasonalDeviation,getScatterChart,getMonthlyDeviationLine } from "@/api/statisticApi";
...
<template>
    <div>
        ...
        :data="scatterData" style="flex: 1" />
        <LineChart 
        v-if="monthlyLineData.data"
        :data="monthlyLineData.data" style="flex: 1" />
        <Footer />
    </div>
</template>
